...
void find_test(){
    ...
    auto loc = std::find(std::begin(vec),std::end(vec),1); //I want to find an element 1;
    if(loc != std::end(vec))
    ...
    auto loc1 = std::find(players.begin(),players.end(),Person{"Moe",12});
    if(loc1!=players.end())
...
void count_if_test(){
    ...
    //how can we determine how many elements
    //use lambda
