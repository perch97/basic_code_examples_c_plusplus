...
void test3(){
    ...
    //All a constant iterator is an iterator but I can't change what it's pointing to.
    std::cout<<"\n========================="<<std::endl;
    std::vector<int> nums1 {1,2,3,4,5};
    ...
    //compile error when we try to change element
    it1 = nums1.begin();
    while(it1 != nums1.end()){
...
void test4(){
    ...
    while(it1 != vec.rend()){
        std::cout<<*it1<<std::endl;
        it1++; //incrementing here really means decrementing 
    }
    std::cout<<"\n========================="<<std::endl;
    ...
    auto it3 = favourites.begin(); //iterator over map of string, string pairs
